...
def inference(images, batch_size):
    ...
    with tf.variable_scope('conv1') as scope:
        kernel = tf.Variable(tf.random_normal([11, 11, 3, 96], stddev=0.01), name='weights')
        conv = tf.nn.conv2d(images, kernel, [1, 4, 4, 1], padding='SAME')
        biases = tf.Variable(tf.constant(0.0, dtype=tf.float32, shape=[96]), name='biases')
    ...
    with tf.variable_scope('conv2') as scope:
        kernel = tf.Variable(tf.random_normal([5, 5, 96, 128], stddev=0.01), name='weights')
    ...
    with tf.variable_scope('conv3') as scope:
        kernel = tf.Variable(tf.random_normal([3, 3, 128, 192], stddev=0.01), name='weights')
    ...
    with tf.variable_scope('conv4') as scope:
        kernel = tf.Variable(tf.random_normal([3, 3, 192, 192], stddev=0.01), name='weights')
    ...
    with tf.variable_scope('conv5') as scope:
        kernel = tf.Variable(tf.random_normal([3, 3, 192, 128], stddev=0.01), name='weights')
    ...
    # pool3
    pool3 = tf.nn.max_pool(conv5, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
    ...
    with tf.variable_scope('local1') as scope:
        ...
        biases = tf.Variable(tf.constant(0.1, dtype=tf.float32, shape=[2048]), name='biases')
        local1 = tf.nn.relu_layer(reshape, weights, biases, name=scope.name)
        local1 = tf.nn.dropout(local1, 0.6)
    ...
    with tf.variable_scope('local2') as scope:
        ...
        local2 = tf.nn.dropout(local2, 0.6)
